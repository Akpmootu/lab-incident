...
export default function Layout({ children }: { children: ReactNode }) {
      ...
      {navItems.map(item => {
        ...
        return (
          <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)} aria-current={isActive ? 'page' : undefined}
            className={cn('group relative flex items-center gap-3 rounded-xl px-3 py-3 text-sm transition-all duration-200', isActive ? 'bg-teal-50 text-teal-800 shadow-sm ring-1 ring-teal-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900', collapsed && !mobile && 'justify-center px-2')}>
            {isActive && <span className="absolute inset-y-2 left-0 w-1 rounded-r-full bg-teal-600" />}
            <span className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-sm transition-colors', isActive ? 'bg-teal-600 text-white shadow-sm shadow-teal-600/20' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200')}><i className={item.icon} /></span>
  ...
  return (
    <div className="min-h-screen bg-[#f6f8f7] text-slate-900">
      <aside className={cn('fixed inset-y-0 left-0 z-40 hidden border-r border-slate-200/80 bg-white/90 backdrop-blur-xl transition-[width] duration-200 md:flex md:flex-col', collapsed ? 'w-[84px]' : 'w-[264px]')}>
        <div className={cn('flex h-[76px] items-center border-b border-slate-100 px-4', collapsed ? 'justify-center' : 'justify-between')}>
          <Link to="/" className={cn('flex items-center gap-3', collapsed && 'justify-center')}>
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-950 text-teal-300 shadow-lg shadow-slate-900/15"><i className="fa-solid fa-microscope" /></span>
        ...
        <div className={cn('border-t border-slate-100 p-4', collapsed && 'px-3')}>
          <div className={cn('flex items-center gap-3 rounded-xl bg-slate-50 p-3', collapsed && 'justify-center p-2')}>
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal-100 text-teal-700"><i className="fa-solid fa-user-doctor" /></span>
      ...
      <AnimatePresence>{mobileOpen && <><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileOpen(false)} className="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm md:hidden" /><motion.aside initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ duration: .2 }} className="fixed inset-y-0 left-0 z-50 w-[280px] bg-white md:hidden"><div className="flex h-[76px] items-center justify-between border-b border-slate-100 px-5"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-950 text-teal-300"><i className="fa-solid fa-microscope" /></span><b>Lab Incident</b></div><button onClick={() => setMobileOpen(false)} className="icon-button" aria-label="ปิดเมนู"><i className="fa-solid fa-xmark" /></button></div><Nav mobile /></motion.aside></>}</AnimatePresence>
